...
import { PROFILE_URL, YOUTUBE_SEARCH_API } from "../utils/constants";
 ...
 *
 * key - ip
 * - cleanup of previous useEffect runs
    ...
    <div className="grid grid-flow-col p-7 shadow-lg items-center">
      ...
      <div className="col-span-10 px-10">
        <div>
          <input
            type="text"
            className="w-2/3 px-2 py-2 border border-gray-400 rounded-l-full"
            value={searchQuery}
        ...
        {showSuggestions && (
          <div className="fixed bg-white py-2 w-[47rem] rounded-lg shadow-2xl mt-2">
            <ul className="flex flex-col gap-2.5">
              {suggestions.map((suggestion) => (
                <li
                  key={suggestion}
                  className="py-1.5 px-2 hover:px-3 hover:bg-gray-100 transition-all"
                >
                  🔍 {suggestion}
                </li>
              ))}
      ...
      <div className="col-span-1">
        <img className="h-8" src={PROFILE_URL} alt="user profile" />
      </div>
    </div>
  );
};
